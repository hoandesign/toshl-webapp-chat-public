...
import { Settings, SendHorizonal, Loader2, ImagePlus, X, Bug, Play, Pause } from 'lucide-react';
...
type EntrySuccessGroup = { type: 'entry_success_group'; entries: Message[]; id: string };
type BudgetGroup = { type: 'budget_group'; budgets: Message[]; id: string };
type MessageWithHistoryGroup = { type: 'message_with_history_group'; message: Message; group: HistoryGroup; id: string };
type MessageWithBudgetGroup = { type: 'message_with_budget_group'; message: Message; group: BudgetGroup; id: string };
type ProcessedItem = Message | HistoryGroup | AccountBalanceGroup | BudgetGroup | EntrySuccessGroup | MessageWithHistoryGroup | MessageWithBudgetGroup;

const processMessagesForRendering = (messages: Message[]): ProcessedItem[] => {
  const processedItems: ProcessedItem[] = [];
  let currentHistoryGroup: Message[] = [];
  let currentAccountGroup: Message[] = [];
  let currentBudgetGroup: Message[] = [];

  // First pass: group consecutive entries/accounts/budgets.
  ...
  // Second pass: combine header message + immediately following group.
  ...
  for (let i = 0; i < processedItems.length; i++) {
    ...
    if ('sender' in currentItem) {
      if (currentItem.type === 'history_header' && nextItem?.type === 'history_group') {
        ...
        i++;
      } else if (currentItem.type === 'budget_header' && nextItem?.type === 'budget_group') {
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ toggleSettings, hideNumbers }): React.ReactElement => { // Accept hideNumbers prop
  ...
  const processedRenderItems = useMemo(() => processMessagesForRendering(messages), [messages]);

  const renderedMessages = useMemo(() => {
    return processedRenderItems.map((item) => {
      ...
      // --- Render Standalone Entry Success Group ---
      else if (item.type === 'entry_success_group') {
        return (
          <div key={item.id} className="mb-4">
            <div className={`grid grid-flow-col auto-cols-max items-start gap-x-3 py-2 -mx-4 px-4 ${item.entries.length > 1 ? 'overflow-x-auto scrollbar-thin scrollbar-thumb-primary/40 hover:scrollbar-thumb-primary/60 scrollbar-track-border-color/30 snap-x snap-mandatory' : ''}`}>
              {item.entries.map((successMsg) => (
                <div key={successMsg.id} className={`${item.entries.length > 1 ? 'snap-center w-[85vw] max-w-sm' : 'w-full'} shrink-0`}>
                  <MessageRenderer
                    msg={successMsg}
                    handleDeleteEntry={handleDeleteEntryLocally}
                    handleDeleteMessageLocally={handleDeleteMessageOnlyLocally}
                    handleRetryOfflineMessage={handleRetryOfflineMessage}
    ...
    hideNumbers,
    handleShowMoreClick,
    handleShowMoreAccountsClick,
  ]);

  return (
    <div ref={chatContainerRef} className="toshl-main-container relative h-full overflow-hidden"> {/* Main container like chatscope */}
      <div className="toshl-chat-container flex flex-col h-full overflow-hidden"> {/* Chat container that handles scrolling */}
        ...
        <div className="toshl-message-list flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-btn-red/50 hover:scrollbar-thumb-btn-red/70 scrollbar-track-separator-gray/50 p-4 md:p-6 space-y-6 min-h-0 flex flex-col"> {/* Flexible message area that grows */}
          {renderedMessages}
        ...
        <div className="toshl-message-input flex-shrink-0 relative pt-3 px-3 md:pt-4 md:px-4 bg-card-bg shadow-inner z-[5]"> {/* Fixed input area */}
        {isMentionPopupOpen && mentionSuggestions.length > 0 && !isRecording && (
          <MentionSuggestionsPopup
            suggestions={mentionSuggestions}
            onSelect={(suggestion) => {
              handleMentionSelect(suggestion, (newCursorPosition) => {
                if (textareaRef.current) {
