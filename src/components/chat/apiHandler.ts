...
// --- Helper to get required data from IndexedDB ---
const getRequiredApiData = async () => {
    const [currencyRaw, geminiModelRaw, toshlDataFetched, accountsStr, categoriesStr, tagsStr] = await Promise.all([
        idbStorage.getItem('currency'),
        idbStorage.getItem('geminiModel'),
        idbStorage.getItem('toshlDataFetched'),
        idbStorage.getItem('toshlAccounts'),
        idbStorage.getItem('toshlCategories'),
        idbStorage.getItem('toshlTags'),
    ]);

    const currency = currencyRaw || STRINGS.DEFAULT_CURRENCY_VALUE;
    const geminiModel = geminiModelRaw || STRINGS.DEFAULT_GEMINI_MODEL_VALUE;

    // Decrypt the API keys
    const [toshlApiKey, geminiApiKey] = await Promise.all([
        getDecryptedApiKey('toshlApiKey'),
        getDecryptedApiKey('geminiApiKey'),
    ]);
...
export const handleProcessUserRequestApi = async (
    ...
    try {
        const { toshlApiKey, geminiApiKey, currency, geminiModel, accounts, categories, tags } = await getRequiredApiData();
    ...
    switch (geminiResult.action) {
        case 'add': {
            ...
            const payloads = geminiResult.payloads || [];

            // For editing, we usually just update the last added entry.
            // In case of multiple additions, we'll store the ID of the last added entry in the array.

            for (let i = 0; i < payloads.length; i++) {
                const toshlPayload = payloads[i];
                ...
                if (finalEntryId && finalEntryId !== STRINGS.TOSHL_ENTRY_CREATED_NO_ID) {
                    newLastSuccessfulEntryId = finalEntryId; // This will correctly capture the very last ID added in the array
                } else if (i === payloads.length - 1) { // If last item and no ID, set to null
                    newLastSuccessfulEntryId = null;
                }

                messagesToAdd.push({
                    id: `success_${finalEntryId || Date.now()}_${i}`,
                    sender: 'bot',
                    type: 'entry_success',
                    entryData: successEntryData,
                    debugInfo: i === 0 ? geminiDebugInfo : undefined // Only attach debug info to the first success card to avoid duplicating large JSONs
                });
            }
            break;
        }
...
export const handleDeleteEntryApi = async (toshlEntryId: string): Promise<void> => {
    const { toshlApiKey } = await getRequiredApiData(); // Only need Toshl key
