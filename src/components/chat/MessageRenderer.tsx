...
const AudioDisplay: React.FC<{
    ...
    React.useEffect(() => {
        ...
        if (rawDuration && duration === 0) {
            // Delay state update slightly to avoid synchronous setState during render phase
            // Workaround for React 19 strict effects / lint warnings; defer state sync
            defer(() => {
                if (isMounted) setDuration(rawDuration / 1000); // Convert from milliseconds to seconds
            });
        }
    ...
    return (
        <div className="flex items-center space-x-3 p-3 bg-navigation-bg border border-btn-red/30 rounded-lg min-w-[160px]">
            <button
                ...
                className="flex-shrink-0 w-8 h-8 bg-btn-red hover:bg-btn-red-highlight text-white rounded-full flex items-center justify-center transition duration-200 touch-manipulation min-h-[44px] min-w-[44px]"
...
const ImageDisplay: React.FC<{
    ...
    React.useEffect(() => {
        ...
        // Delay state updates to avoid synchronous setState during render
        // Workaround for React 19 strict effects / lint warnings; defer state sync
        defer(() => {
            if (!isMounted) return;
                ...
                <div className="flex flex-col items-center space-y-2">
                    ...
                    {retryCount < MAX_RETRY_ATTEMPTS && (
                        <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 bg-btn-red/20 hover:bg-btn-red/30 text-btn-red text-xs font-medium rounded-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-btn-red focus:ring-offset-1 touch-manipulation min-h-[44px]"
...
const MessageRenderer: React.FC<MessageRendererProps> = ({ message: msg, isDeleting, isRetrying, handleDeleteEntry, handleDeleteMessageLocally, retrySendMessage, hideNumbers }) => { // Accept hideNumbers prop
    ...
    // Handle touch events on buttons specifically
    ...
    const confirmEntryDeletion = (entryId?: string) => {
        if (typeof window === 'undefined' || typeof window.confirm !== 'function') {
            return true; // Assume confirmation in non-browser environments (e.g., tests)
        }
        return window.confirm(STRINGS.CONFIRM_DELETE_ENTRY(entryId));
    };
    ...
    if (shouldShowActions) {
        actionButtons = (
            <div className={`absolute -top-8 ${msg.sender === 'user' ? 'right-0' : 'left-0'} flex items-center space-x-1 bg-gray-800/90 backdrop-blur-sm px-2 py-1 rounded-lg shadow-lg transition-all duration-200 ${isMobile ? (showContextMenu ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-2 pointer-events-none') : 'opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto'}`}>
                ...
                {canCopy && canHaveActions && (
                    <button
                        ...
                        onTouchEnd={(e) => handleButtonTouch(e, () => handleCopy(msg.text))}
                        className={`text-gray-300 hover:text-white p-1 rounded transition-colors ${isMobile ? 'touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center active:bg-gray-600' : 'hover:bg-gray-700'}`}
                ...
                {hasDebugInfo && (
                    <button
                        ...
                        onTouchEnd={(e) => handleButtonTouch(e, () => setShowDebugModal(true))}
                        className={`text-gray-300 hover:text-blue-400 p-1 rounded transition-colors ${isMobile ? 'touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center active:bg-gray-600' : 'hover:bg-gray-700'}`}
                ...
                {canDeleteLocally && canHaveActions && (
                    <button
                        ...
                        onTouchEnd={(e) => handleButtonTouch(e, () => handleDeleteMessageLocally(msg.id))}
                        className={`text-gray-300 hover:text-red-400 p-1 rounded transition-colors ${isMobile ? 'touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center active:bg-gray-600' : 'hover:bg-gray-700'}`}
        ...
        case 'entry_success': {
            ...
            if (msg.isDeleted || !successData) {
                ...
                const handleDeleteButtonClick = () => {
                    if (!confirmEntryDeletion(successData.id)) {
                        return;
                    }
                    handleDeleteEntry(msg.id, successData.id!);
                };

                content = (
                    <div className="w-full h-full min-w-[280px] flex flex-col"> {/* Further increased max-width to prevent ID overlap, added min-width */}
                        <div className={`border border-btn-green/30 ${bgColor} rounded-lg shadow-md p-4 w-full h-full animate-fade-in flex flex-col`}> {/* Use lighter border */}
                        ...
                        {successData.id && successData.id !== 'unknown_id' && successData.id !== STRINGS.TOSHL_ENTRY_CREATED_NO_ID && (
                            <div className="flex justify-end mt-2"> {/* Align button to the right */}
                                <button
                                    onClick={handleDeleteButtonClick} // Use API delete
                                    onTouchEnd={(e) => handleButtonTouch(e, handleDeleteButtonClick)}
                                    disabled={isDeleting === msg.id} // Use isDeleting state for this button
                                    className={`flex items-center px-3 py-1.5 bg-btn-red/10 text-btn-red text-sm font-medium rounded-lg hover:bg-btn-red/20 transition duration-150 disabled:opacity-50 disabled:cursor-wait shadow-sm hover:shadow ${isMobile ? 'touch-manipulation min-h-[44px]' : ''}`} // Larger, themed red delete button
                ...
                if (msg.status === 'pending') {
                    ...
                    if (msg.offlineId) {
                        retryButton = (
                            <button
                                ...
                                onTouchEnd={(e) => handleButtonTouch(e, () => retrySendMessage(msg.offlineId!))}
                                disabled={isRetrying === msg.offlineId}
                                className={`text-xs text-navigation-icon hover:text-navigation-text underline ml-2 disabled:opacity-50 disabled:cursor-wait ${isMobile ? 'touch-manipulation min-h-[44px]' : ''}`} // Use navigation theme colors
                                title="Retry sending"
                            >
                    ...
                    retryButton = (
                        <button
                            ...
                            onTouchEnd={(e) => handleButtonTouch(e, () => retrySendMessage(msg.offlineId!))}
                            disabled={isRetrying === msg.offlineId}
                            className={`text-xs text-red-400 hover:text-navigation-text underline ml-2 disabled:opacity-50 disabled:cursor-wait ${isMobile ? 'touch-manipulation min-h-[44px]' : ''}`} // Use red-400 for error, hover to white
                            title="Retry sending"
                        >
                            {isRetrying === msg.offlineId ? <Loader2 size={12} className="animate-spin inline-block" /> : <RefreshCw size={12} className="inline-block" />} Retry
                        </button>
