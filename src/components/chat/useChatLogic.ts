...
interface UseChatLogicReturn {
    ...
    handleMentionSelect: (suggestion: MentionSuggestion, onSelectComplete?: (newCursorPosition: number) => void) => void;
...
export const useChatLogic = (): UseChatLogicReturn => {
    // Start with greeting; load persisted history from IndexedDB on mount.
...
const [mentionSuggestions, setMentionSuggestions] = useState<MentionSuggestion[]>([]);
    ...
    // Load persisted messages + quick-add messages from IndexedDB on mount
    useEffect(() => {
        void (async () => {
            try {
                const [storedMessagesRaw, storedQuickAddRaw] = await Promise.all([
                    idbStorage.getItem(LOCAL_STORAGE_KEY),
                    idbStorage.getItem(QUICK_ADD_MESSAGES_KEY),
                ]);

                if (storedMessagesRaw) {
                    const parsed = JSON.parse(storedMessagesRaw);
                    if (Array.isArray(parsed)) {
                        const filteredMessages = parsed.filter((msg: Message) => msg.type !== 'loading');
                        setMessages(filteredMessages.length > 0
                            ? filteredMessages
                            : [{ id: 'init', text: STRINGS.INITIAL_GREETING, sender: 'system', type: 'system_info', status: 'sent', timestamp: new Date().toISOString() }]);
                    }
                }

                if (storedQuickAddRaw) {
                    setQuickAddMessages(JSON.parse(storedQuickAddRaw));
                }
            } catch (error) {
    ...
    useEffect(() => {
        void (async () => {
            try {
                ...
                await idbStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(messagesToSave));
            } catch (error) {
    ...
    // Save quick add messages to IndexedDB whenever they change
    useEffect(() => {
        void (async () => {
            try {
                await idbStorage.setItem(QUICK_ADD_MESSAGES_KEY, JSON.stringify(quickAddMessages));
            } catch (error) {
                console.error("Failed to save quick add messages to IndexedDB:", error);
            }
        })();
    }, [quickAddMessages]);


    // Effect to load Toshl data from IndexedDB on mount
    useEffect(() => {
        void (async () => {
            try {
                const [storedAccounts, storedCategories, storedTags] = await Promise.all([
                    idbStorage.getItem(TOSHL_ACCOUNTS_KEY),
                    idbStorage.getItem(TOSHL_CATEGORIES_KEY),
                    idbStorage.getItem(TOSHL_TAGS_KEY),
                ]);
                ...
                console.error("Failed to load Toshl data from IndexedDB for mentions:", error);
            }
        })();
    }, []); // Run only on mount
    ...
    const handleFetchDateRange = useCallback(async (fromDate?: string, toDate?: string, days?: number, headerTextFromGemini?: string) => {
        ...
        try {
            ...
            const toshlApiKey = await getDecryptedApiKey('toshlApiKey');
            ...
            const categoriesStr = await idbStorage.getItem('toshlCategories');
            const tagsStr = await idbStorage.getItem('toshlTags');
    ...
    useEffect(() => {
        ...
        if (!isMentionPopupOpen || mentionTriggerIndex === null) {
            ...
            // Avoid interfering with text input caret updates by deferring suggestions to next tick
            defer(() => {
                setMentionSuggestions(currentSuggestions => (currentSuggestions.length > 0 ? [] : currentSuggestions));
            });
            return;
    ...
    const handleMentionSelect = useCallback((suggestion: MentionSuggestion, onSelectComplete?: (newCursorPosition: number) => void) => {
        ...
        const newCursorPosition = mentionTriggerIndex + formattedSuggestion.length + 2; // +1 for '@', +1 for space
        ...
        if (onSelectComplete) {
            // Need to allow React to update the value first before setting selection
            setTimeout(() => {
                onSelectComplete(newCursorPosition);
            }, 0);
        }
    ...
    useEffect(() => {
        // Prevent event listener duplication and execution in non-browser environments
    ...
    const handleClearChatHistory = useCallback(() => {
        if (window.confirm(STRINGS.CLEAR_CHAT_HISTORY_CONFIRM)) {
            void (async () => {
                try {
                    await Promise.all([
                        idbStorage.removeItem(LOCAL_STORAGE_KEY),
                        idbStorage.removeItem('toshlAccounts'),
                        idbStorage.removeItem('toshlCategories'),
                        idbStorage.removeItem('toshlTags'),
                        idbStorage.removeItem('toshlDataFetched'),
                        idbStorage.removeItem(QUICK_ADD_MESSAGES_KEY),
                    ]);
                    console.log("Chat history and related data cleared from IndexedDB.");
