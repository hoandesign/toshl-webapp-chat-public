...
import { X, Loader2, Trash2, RotateCcw } from 'lucide-react';
    ...
    <div className={`flex flex-col bg-app-bg ${isModal ? 'h-full' : ''}`}> {/* Changed main bg to app-bg for consistency */}
       ...
       <form className="flex-1 overflow-y-auto p-4 md:p-8 space-y-5 bg-card-bg" onSubmit={(e) => { e.preventDefault(); handleSave(); }}> {/* Added bg-card-bg here */}
        ...
        <div className="space-y-1">
          <label htmlFor="securityPin" className="block text-sm font-medium text-black-text">
            Security PIN
          </label>
          <input
            type="password"
            ...
            className="w-full px-4 py-2 border border-separator-gray rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition duration-200 bg-input-bg text-black-text"
            placeholder="Enter a PIN to encrypt keys"
            required
          />
          <p className="text-xs text-gray-text pt-1">This PIN encrypts your keys locally. You will need it to unlock the app.</p>
        </div>
