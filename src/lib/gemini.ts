...
export async function processUserRequestViaGemini( // Renamed function
    ...
    // Cache toggle based on user settings (stored in IndexedDB)
    const cacheEnabled = await idbStorage.getJSON<boolean>('useGeminiCache', false);
                    ...
                    if (!payloads || !Array.isArray(payloads) || payloads.length === 0) {
                        ...
                        for (const payload of payloads) {
                            if (
                                ...
                                (payload.tags && (!Array.isArray(payload.tags) || !payload.tags.every((t: string) => typeof t === 'string')))
                            ) {
                                hasError = true;
                                break;
                            }
