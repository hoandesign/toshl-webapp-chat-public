...
import { encryptApiKey, setSessionPin } from '../utils/encryption';
...
describe('API Handler - Image Message Processing', () => {
  const setupStorage = async (overrides: Record<string, string | null>) => {
    const defaultStore: Record<string, string | null> = {
      toshlApiKey: encryptApiKey('test-toshl-key', '1234'),
      geminiApiKey: encryptApiKey('test-gemini-key', '1234'),
      currency: 'USD',
    ...
    await Promise.all(
      Object.entries(store)
        .filter(([, v]) => v !== null)
        .map(([k, v]) => idbStorage.setItem(k, v as string))
    );
  };
  ...
  describe('Error handling for image processing', () => {
    it('should handle API errors gracefully when processing image messages', async () => {
      ...
      try {
        ...
        const err = e as { messagesToAdd?: Message[] };
        expect(err.messagesToAdd).toBeInstanceOf(Array);
        expect(err.messagesToAdd?.length).toBe(1);
        expect(err.messagesToAdd?.[0].text).toBe(expectedError.message);
        expect(err.messagesToAdd?.[0].type).toBe('error');
      }
    });

    it('should handle missing API keys', async () => {
      await setupStorage({ geminiApiKey: null });
      ...
      try {
        ...
        expect(err.messagesToAdd).toBeInstanceOf(Array);
        expect(err.messagesToAdd?.length).toBe(1);
        expect(err.messagesToAdd?.[0].text).toBe(STRINGS.API_KEYS_NOT_CONFIGURED);
        expect(err.messagesToAdd?.[0].type).toBe('error');
      }
    });

    it('should handle missing Toshl data', async () => {
      await setupStorage({ toshlDataFetched: null });
      const messages: Message[] = [];
      try {
        await handleProcessUserRequestApi('Test message', messages, null, null);
        fail('Expected handleProcessUserRequestApi to reject');
        ...
        expect(err.messagesToAdd).toBeInstanceOf(Array);
        expect(err.messagesToAdd?.length).toBe(1);
        expect(err.messagesToAdd?.[0].text).toBe(STRINGS.TOSHL_DATA_NOT_FETCHED);
        expect(err.messagesToAdd?.[0].type).toBe('error');
      }
    });
  });
